...
def openrouter_request_endpoint():
    """Handle the OpenRouter request endpoint"""
    try:
        ...
        # Get system prompt if provided, otherwise use default
        system_prompt = data.get("system_prompt", "You are a helpful assistant.")
        
        # Make request to OpenRouter
        result, status_code = lessons_manager.openrouter_request(data["prompt"], system_prompt)
        
        return jsonify(result), status_code
        
    except Exception as e:
        logger.error(f"Error in openrouter_request_endpoint: {str(e)}")
        return jsonify({"error": f"Error making OpenRouter request: {str(e)}"}), 500