...
def openrouter_request(prompt: str, system_prompt: str = "You are a helpful assistant.") -> Tuple[Dict, int]:
    """Make a generic request to OpenRouter API with the default model"""
    try:
        if not OPENROUTER_API_KEY:
            return {
                "success": False,
        ...
        logger.info(f"Sending request to OpenRouter with prompt: {prompt[:50]}...")
        ...
        return {
            "success": False,
            "error": f"Error making OpenRouter request: {str(e)}"
        }, 500
